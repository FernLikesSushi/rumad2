import type { TuiScreen } from "./types";

// Fabricated, not scraped -- realistic shapes for visually checking each
// screen component without a live SSH session.
export const mockScreens = {
  Menu: {
    kind: "Menu",
    menu: "MainMenu",
    options: [
      { key: "1", label: "Seleccion de Secciones  (Matricula)" },
      { key: "2", label: "Ver otra informacion" },
      { key: "3", label: "Curriculo" },
      { key: "0", label: "SALIR DEL SISTEMA" },
    ],
  },
  Matricula: {
    kind: "Matricula",
    courses: [
      { slot: "1", course: "MATE3031", section: "030", credits: "4", status: "Matriculado" },
      { slot: "2", course: "INGL3103", section: "045", credits: "3", status: "Matriculado" },
      { slot: "3", course: "HIST3220", section: "032", credits: "3", status: "Baja" },
    ],
    mode: {
      kind: "Actions",
      options: [
        { key: "A", label: "Alta" },
        { key: "B", label: "Baja" },
        { key: "C", label: "Cambio" },
        { key: "H", label: "HorEst" },
        { key: "S", label: "Salir" },
      ],
    },
  },
  CourseResults: {
    kind: "CourseResults",
    courseCode: "HIST 3220",
    courseTitle: "INT HIST:ENF SOCIAL",
    sections: [
      {
        section: "032",
        room: "CH316",
        schedule: "MJ       8:00- 9:15",
        credits: 3,
        professor: "J PEREZ RIVERA",
        capacity: 32,
        used: 32,
        available: 0,
        meetings: [
          { day: 2, startMinutes: 8 * 60, endMinutes: 9 * 60 + 15 },
          { day: 4, startMinutes: 8 * 60, endMinutes: 9 * 60 + 15 },
        ],
      },
      {
        section: "045",
        room: "CH120",
        schedule: "LWV     10:00-10:50",
        credits: 3,
        professor: "M TORRES DIAZ",
        capacity: 30,
        used: 25,
        available: 5,
        meetings: [
          { day: 1, startMinutes: 10 * 60, endMinutes: 10 * 60 + 50 },
          { day: 3, startMinutes: 10 * 60, endMinutes: 10 * 60 + 50 },
          { day: 5, startMinutes: 10 * 60, endMinutes: 10 * 60 + 50 },
        ],
      },
      {
        section: "060",
        room: "S 121",
        schedule: "LWV      2:30- 3:20pm",
        credits: 3,
        professor: "R RIVERA COLON",
        capacity: 25,
        used: 20,
        available: 5,
        meetings: [
          { day: 1, startMinutes: 14 * 60 + 30, endMinutes: 15 * 60 + 20 },
          { day: 3, startMinutes: 14 * 60 + 30, endMinutes: 15 * 60 + 20 },
          { day: 5, startMinutes: 14 * 60 + 30, endMinutes: 15 * 60 + 20 },
        ],
      },
      {
        // Same slot as "032" -- exercises overlapping boxes/color cycling.
        section: "095",
        room: "CH210",
        schedule: "MJ       8:00- 9:15",
        credits: 3,
        professor: "L SANTIAGO",
        capacity: 28,
        used: 28,
        available: 0,
        meetings: [
          { day: 2, startMinutes: 8 * 60, endMinutes: 9 * 60 + 15 },
          { day: 4, startMinutes: 8 * 60, endMinutes: 9 * 60 + 15 },
        ],
      },
      {
        // Evening section past the calendar's default 7pm bound.
        section: "099",
        room: "CH130",
        schedule: "MJ       7:30- 8:45pm",
        credits: 3,
        professor: "D FIGUEROA REYES",
        capacity: 30,
        used: 12,
        available: 18,
        meetings: [
          { day: 2, startMinutes: 19 * 60 + 30, endMinutes: 20 * 60 + 45 },
          { day: 4, startMinutes: 19 * 60 + 30, endMinutes: 20 * 60 + 45 },
        ],
      },
      {
        // "Por acuerdo" -- no fixed room/time, so no meetings to plot.
        section: "005D",
        room: "",
        schedule: "Por acuerdo",
        credits: 3,
        professor: "A MORALES CRUZ",
        capacity: 45,
        used: 40,
        available: 5,
        meetings: [],
      },
    ],
  },
  ConfirmedSchedule: {
    kind: "ConfirmedSchedule",
    courses: [
      {
        course: "BIOL 3045",
        section: "1",
        credits: 0,
        room: "S 104A",
        professor: "",
        meetings: [{ day: 3, startMinutes: 12 * 60 + 30, endMinutes: 14 * 60 + 20 }],
      },
      {
        course: "EDFI 3645",
        section: "041",
        credits: 2,
        room: "CM210",
        professor: "FIGUEROA ROBLES, IRIS A.",
        meetings: [
          { day: 1, startMinutes: 10 * 60, endMinutes: 10 * 60 + 50 },
          { day: 3, startMinutes: 10 * 60, endMinutes: 10 * 60 + 50 },
        ],
      },
      {
        course: "FILO 4045",
        section: "020",
        credits: 3,
        room: "CH119",
        professor: "THE LORAX",
        meetings: [
          { day: 1, startMinutes: 8 * 60 + 30, endMinutes: 9 * 60 + 20 },
          { day: 3, startMinutes: 8 * 60 + 30, endMinutes: 9 * 60 + 20 },
          { day: 5, startMinutes: 8 * 60 + 30, endMinutes: 9 * 60 + 20 },
        ],
      },
      {
        course: "INGE 3045",
        section: "086",
        credits: 3,
        room: "S 305",
        professor: "CAVE JOHNSON",
        meetings: [
          { day: 2, startMinutes: 14 * 60, endMinutes: 15 * 60 + 15 },
          { day: 4, startMinutes: 14 * 60, endMinutes: 15 * 60 + 15 },
        ],
      },
    ],
  },
  WeeklySchedule: {
    kind: "WeeklySchedule",
    days: ["Lunes", "Martes", "Miercoles", "Jueves", "Viernes", "Sabado"],
    rows: [
      { period: "8:30- 9:20", days: ["MATE3031 - 030", "", "MATE3031 - 030", "", "MATE3031 - 030", ""] },
      { period: "10:00-10:50", days: ["", "INGL3103 - 045", "", "INGL3103 - 045", "", ""] },
    ],
  },
  Search: { kind: "Search", search: "HorarioCurso" },
  Unknown: {
    kind: "Unknown",
    raw: "*** Pantalla no reconocida ***\n\n1) Opcion 1\n2) Opcion 2",
    options: [
      { key: "1", label: "Opcion 1" },
      { key: "2", label: "Opcion 2" },
    ],
  },
} satisfies Record<Exclude<TuiScreen["kind"], "Login" | "Disconnected">, TuiScreen>;

...
**`screens/` is one submodule per screen type** (`login.rs`, `select_period.rs`, `matricula.rs`, `notice.rs`, plus a `scrape.rs` for cross-screen scraping helpers), each owning its own types and tests. The one thing that stays in `mod.rs` rather than moving into a submodule is `classify()` itself — see below for why.
